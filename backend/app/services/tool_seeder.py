...
BUILTIN_TOOLS = [
    ...
    },
    {
        ...
        "description": "发送「字段对」飞书卡片到当前飞书会话（仅当前会话，无法指定其他人/其他群）。适合状态汇报、OKR 同步、属性展示。CardKit 渲染失败会自动降级为 markdown 消息。在非飞书会话（Web 调试、非飞书触发器）调用会报错。",
        "category": "feishu",
        "icon": "🪪",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "description": "发送「正文 + 按钮组」飞书卡片到当前飞书会话（仅当前会话，无法指定其他人/其他群）。用户点击按钮时，Agent 会收到一条新的 user 消息 `[卡片操作] {label}`（含可选 context）。适合给用户提供 1-4 个备选动作，无需打字。在非飞书会话调用会报错。",
        "category": "feishu",
        "icon": "🔘",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "description": "发送「表格」飞书卡片到当前飞书会话（仅当前会话，无法指定其他人/其他群），用于展示对比、列表型数据。每行单元格数应与 columns 数量一致。在非飞书会话调用会报错。",
        "category": "feishu",
        "icon": "📋",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "description": "发送「通过 / 拒绝」审批卡片到当前飞书会话（仅当前会话，无法指定其他人/其他群）。用户点击后，Agent 会收到 `[卡片操作] 通过（approval_id=...）` 或对应的拒绝消息。适合需要人类显式 go/no-go 决策的场景（不可逆操作前、对外发送前等）。在非飞书会话调用会报错。",
        "category": "feishu",
        "icon": "🟢",
        "is_default": False,
        "parameters_schema": {
