...
BUILTIN_TOOLS = [
    ...
    # They are NOT enabled by default — agents with Feishu channels should enable them.
    {
        ...
        "description": "Send a Feishu IM message to a colleague or a group chat. For individuals: provide member_name or user_id. For groups: provide chat_id (from feishu_chat_search) or chat_name (auto-searched).",
        "category": "feishu",
        "icon": "💬",
        "is_default": False,
        "parameters_schema": {
            "type": "object",
            "properties": {
                "member_name": {"type": "string", "description": "Recipient's name, e.g. '张三'. Will be looked up automatically."},
                "user_id": {"type": "string", "description": "Recipient's Feishu user_id (preferred, tenant-stable). Get from feishu_user_search."},
                "chat_id": {"type": "string", "description": "Group chat_id (from feishu_chat_search). Mutually exclusive with member_name/user_id."},
                "chat_name": {"type": "string", "description": "Group name to search for. The tool auto-resolves it to a chat_id."},
    ...
    },
    {
        ...
        "display_name": "Feishu Card · KV",
        "description": "发送「字段对」飞书卡片。默认收件人是当前飞书会话；也可通过 member_name / user_id（个人）或 chat_id / chat_name（群聊）指定其他收件人，语义与 Feishu Message 一致。适合状态汇报、OKR 同步、属性展示。CardKit 渲染失败会自动降级为 markdown 消息。未指定收件人且不在飞书会话中（Web 调试、非飞书触发器）时会报错。",
        "category": "feishu",
        "icon": "🪪",
        "is_default": False,
        "parameters_schema": {
            ...
            "properties": {
                ...
                "fields": {
                    ...
                    "items": {
                        "type": "object",
                        "properties": {
                            "key": {"type": "string", "description": "字段名，如「状态」"},
                            "value": {"type": "string", "description": "字段值"},
                            "color": {"type": "string", "description": "可选值颜色：green / red / orange 等"},
                        },
                        "required": ["key", "value"],
                    },
                },
                "summary": {"type": "string", "description": "聊天列表预览文案，可选"},
                "member_name": {"type": "string", "description": "可选：通过姓名指定个人收件人（走关系网关）。不传则发到当前会话。"},
                "user_id": {"type": "string", "description": "可选：通过飞书 user_id 指定个人收件人。不传则发到当前会话。"},
                "chat_id": {"type": "string", "description": "可选：通过 chat_id 指定群聊收件人。不传则发到当前会话。"},
                "chat_name": {"type": "string", "description": "可选：通过群名指定群聊（自动解析为 chat_id）。不传则发到当前会话。"},
            },
            "required": ["fields"],
        },
        "config": {},
    ...
    },
    {
        ...
        "display_name": "Feishu Card · Actions",
        "description": "发送「正文 + 按钮组」飞书卡片。默认收件人是当前飞书会话；也可通过 member_name / user_id（个人）或 chat_id / chat_name（群聊）指定其他收件人，语义与 Feishu Message 一致。用户点击按钮时，Agent 会收到一条新的 user 消息 `[卡片操作] {label}`（含可选 context）。适合给用户提供 1-4 个备选动作，无需打字。未指定收件人且不在飞书会话中时会报错。",
        "category": "feishu",
        "icon": "🔘",
        "is_default": False,
        "parameters_schema": {
            ...
            "properties": {
                "title": {"type": "string", "description": "卡片标题，可选"},
                "body": {"type": "string", "description": "按钮上方的 markdown 正文"},
                "actions": {
                    "type": "array",
                    "description": "1-4 个按钮，按显示顺序",
                    "items": {
                        "type": "object",
                        "properties": {
                            "label": {"type": "string", "description": "按钮文案，≤30 字"},
                            "action_id": {"type": "string", "description": "稳定 id，回调时回传，用于路由/幂等"},
                            "style": {"type": "string", "enum": ["primary", "default", "danger", "text"], "description": "按钮样式"},
                            "context": {"type": "object", "description": "可选结构化负载，点击时序列化注入合成消息"},
                            "confirm": {
                                "type": "object",
                                "description": "点击前的确认弹窗，可选",
                                "properties": {
                                    "title": {"type": "string"},
                                    "text": {"type": "string"},
                                },
    ...
    },
    {
        ...
        "display_name": "Feishu Card · Table",
        "description": "发送「表格」飞书卡片。默认收件人是当前飞书会话；也可通过 member_name / user_id（个人）或 chat_id / chat_name（群聊）指定其他收件人，语义与 Feishu Message 一致。用于展示对比、列表型数据。每行单元格数应与 columns 数量一致。未指定收件人且不在飞书会话中时会报错。",
        "category": "feishu",
        "icon": "📋",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "display_name": "Feishu Card · Approval",
        "description": "发送「通过 / 拒绝」审批卡片。默认收件人是当前飞书会话；也可通过 member_name / user_id（个人）或 chat_id / chat_name（群聊）指定其他审批人，语义与 Feishu Message 一致。用户点击后，Agent 会收到 `[卡片操作] 通过（approval_id=...）` 或对应的拒绝消息。适合需要人类显式 go/no-go 决策的场景（不可逆操作前、对外发送前等）。未指定收件人且不在飞书会话中时会报错。",
        "category": "feishu",
        "icon": "🟢",
        "is_default": False,
        "parameters_schema": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "description": "卡片标题，如「需要您的审批」"},
                "summary_text": {"type": "string", "description": "解释正在审批什么的 markdown 正文"},
                "approval_id": {"type": "string", "description": "稳定 id，回调中回传，用于路由/幂等"},
                "approve_label": {"type": "string", "description": "通过按钮文案，默认「通过」"},
                "reject_label": {"type": "string", "description": "拒绝按钮文案，默认「拒绝」"},
                "summary": {"type": "string"},
                "member_name": {"type": "string", "description": "可选：通过姓名指定个人收件人（走关系网关）。不传则发到当前会话。"},
                "user_id": {"type": "string", "description": "可选：通过飞书 user_id 指定个人收件人。不传则发到当前会话。"},
                "chat_id": {"type": "string", "description": "可选：通过 chat_id 指定群聊收件人。不传则发到当前会话。"},
    ...
    # --- Feishu Chat Tools ---
    {
        ...
        "display_name": "Feishu Chat Search",
        "description": "Search for Feishu group chats visible to the bot or user. Returns chat_id, name, description, and member count for each matching group.",
        "category": "feishu",
        "icon": "🔎",
        "is_default": False,
        "parameters_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Search keyword (group name, member name, or pinyin). Max 64 characters."},
    ...
    },
    {
        ...
        "display_name": "Feishu Chat Messages",
        "description": "Retrieve historical messages from a Feishu group chat. Requires chat_id (get it from feishu_chat_search first). Only text message content is returned; other types are shown as placeholders.",
        "category": "feishu",
        "icon": "📨",
        "is_default": False,
        "parameters_schema": {
            "type": "object",
            "properties": {
                "chat_id": {"type": "string", "description": "Group chat ID (obtained from feishu_chat_search)."},
                "start_time": {"type": "string", "description": "Start time in ISO 8601 format (e.g. 2024-01-15T09:00:00+08:00). Optional."},
                "end_time": {"type": "string", "description": "End time in ISO 8601 format. Optional."},
                "page_size": {"type": "integer", "description": "Messages per page, default 50, max 50."},
                "sort_type": {"type": "string", "enum": ["ByCreateTimeAsc", "ByCreateTimeDesc"], "description": "Sort order. Default ByCreateTimeDesc (newest first)."},
            },
            "required": ["chat_id"],
        },
        "config": {},
