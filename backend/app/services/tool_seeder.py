...
BUILTIN_TOOLS = [
    ...
    },
    {
        ...
        "description": "发送一张「字段对」飞书卡片：标题 + 多行 key/value 字段。适合状态汇报、OKR 同步、属性展示。三选一指定收件人：member_name / direct_user_id / chat_id。CardKit 渲染失败会自动降级为 markdown 消息。",
        "category": "feishu",
        "icon": "🪪",
        "is_default": False,
        "parameters_schema": {
            "type": "object",
            "properties": {
                "member_name": {"type": "string", "description": "关系网络里的人名"},
                "direct_user_id": {"type": "string", "description": "飞书 user_id（tenant-stable）"},
    ...
    },
    {
        ...
        "description": "发送一张「正文 + 按钮组」飞书卡片。用户点击按钮时，Agent 会收到一条新的 user 消息 `[卡片操作] {label}`（含可选 context）。适合给用户提供 1-4 个备选动作，无需打字。三选一指定收件人。",
        "category": "feishu",
        "icon": "🔘",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "description": "发送一张「表格」飞书卡片，用于展示对比、列表型数据。每行单元格数应与 columns 数量一致。三选一指定收件人。",
        "category": "feishu",
        "icon": "📋",
        "is_default": False,
        "parameters_schema": {
    ...
    },
    {
        ...
        "description": "发送一张「通过 / 拒绝」审批卡片。用户点击后，Agent 会收到 `[卡片操作] 通过（approval_id=...）` 或对应的拒绝消息。适合需要人类显式 go/no-go 决策的场景（不可逆操作前、对外发送前等）。三选一指定收件人。",
        "category": "feishu",
        "icon": "🟢",
        "is_default": False,
        "parameters_schema": {
