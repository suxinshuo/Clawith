 ...
 *
 * Ported from the fork's `convertFrontmatter` (previously implemented via
 * remarkFrontmatter). Since the renderer no longer depends on react-markdown,
 * frontmatter is handled directly on the source string:
 *   - `showFrontmatter` → convert the block into a visible ```yaml code block
 *     (used to display soul.md / memory.md).
 *   - otherwise → strip the block, matching remarkFrontmatter's default.
 */
...
interface MarkdownRendererProps {
    ...
    className?: string;
    /** When true, leading YAML frontmatter is rendered as a visible code block instead of being stripped */
    showFrontmatter?: boolean;
}

export const MarkdownRenderer = React.memo(function MarkdownRenderer({ content, mentionNames = [], style, className, showFrontmatter }: MarkdownRendererProps) {
    const html = useMemo(() => {
        const source = showFrontmatter ? convertFrontmatter(content) : stripFrontmatter(content);
        return markdownToHtml(source, mentionNames);
    }, [content, mentionNames, showFrontmatter]);
